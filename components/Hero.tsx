...
// ── Carousel — exported separately so it can be its own page section ──────────
export function CarouselSection({ projects = [], scrollSpeed = "medium", cardTitleTypography, cardCategoryTypography }: CarouselProps) {
  ...
  useEffect(() => {
    ...
    origCards.forEach(card => {
      ...
      clone.style.animation = "none"; clone.style.opacity = "1"; clone.style.clipPath = "none";
    ...
    const FRICTION = 0.88;
    let x = -280, halfW = 0, dragging = false, pointerStartX = 0, xAtDragStart = 0;
    ...
    function loop() {
      ...
      else if (isCoasting) { vx *= FRICTION; x += vx; if (Math.abs(vx) < 0.15) { isCoasting = false; vx = 0; } }
      applyX(); rafId = requestAnimationFrame(loop);
    }
          ...
          {projects.length > 0 ? projects.map(p => (
            <div className="project-card" key={p._id}>
              {p.coverImage ? (
                ...
                <div className="card-placeholder"><span className="card-num">??</span><span className="card-label">No Image</span></div>
              )}
              <div className="card-overlay">
                <h3 style={typoStyle(cardTitleTypography)}>{p.title}</h3>
                <p style={typoStyle(cardCategoryTypography)}>{p.category}</p>
              </div>
            </div>
          )) : placeholders.map(p => (
            <div className="project-card" key={p.num}>
              <div className="card-placeholder"><span className="card-num">{p.num}</span><span className="card-label">Add Image</span></div>
              <div className="card-overlay">
                <h3 style={typoStyle(cardTitleTypography)}>{p.title}</h3>
                <p style={typoStyle(cardCategoryTypography)}>{p.cat}</p>
              </div>
            </div>
          ))}
        </div>
...
// ── Hero title block ──────────────────────────────────────────────────────────
export default function Hero({ headlineTop = "CROSSMEDIA", headlineBottom = "Designer", projects = [], topLineTypography, bottomLineTypography }: HeroProps) {
  return (
    <>
      <style>{`
